...
namespace bustub
{
  ...
  protected:
    ...
    const size_t pool_size_; // 缓冲区是以frame为单位的，pool_size_指的是frame的数量
    /** The next page id to be allocated  */
    std::atomic<page_id_t> next_page_id_ = 0; // ❓暂时没有明白next_page_id_的意义
    /** Bucket size for the extendible hash table */
    const size_t bucket_size_ = 4; // 可扩展哈希桶的大小为4

    /** Array of buffer pool pages. */
    Page *pages_;
    /** Pointer to the disk manager. */
    DiskManager *disk_manager_ __attribute__((__unused__));
    /** Pointer to the log manager. Please ignore this for P1. Project #1可以暂时忽略*/
    ...
    std::list<frame_id_t> free_list_; // 空闲的frame的链表
  ...
  private:
    void SetMetadata_UpdateTableAndCache(Page &page, page_id_t page_id, frame_id_t frame_id);
  };
} // namespace bustub
