...
namespace bustub
{
  ...
  public:
    ...
    {
      /* 方法一 */
      return !(this->children_.begin() == this->children_.end());
    ...
    {
      auto val = this->children_.find(key_char);
      if (val == this->children_.end())
      {
        return nullptr;
      }
      return &(val->second);
  ...
  public:
    ...
    bool Insert(const std::string &key, T value)
    {
      ...
      /* 如果 end_char 已经存在 */
      if (cur->get()->HasChild(end_char))
      {
        auto end_ptr = cur->get()->GetChildNode(end_char);
        /* 如果重复插入 */
        if (end_ptr->get()->IsEndNode())
        {
          latch_.WUnlock();
          return false;
        }
        cur->get()->RemoveChildNode(end_char);
        auto end_node = std::move(*end_ptr);
        auto temp = std::make_unique<TrieNodeWithValue<T>>((std::move(*end_node)), value);
        cur->get()->InsertChildNode(end_char, std::move(temp));
      }
      else
      {
        auto temp = std::make_unique<TrieNodeWithValue<T>>(end_char, value);
        cur->get()->InsertChildNode(end_char, std::move(temp));
      }
      latch_.WUnlock();
      return true;
     ...
     * of another key.
     *
     * 移除操作是从末尾节点到Trie树根节点进行的，只能移除没有孩子且不是其他键的末尾节点的节点
     *
     * @param key Key used to traverse the trie and find the correct node
     * @return True if the key exists and is removed, false otherwise
     */
    /* Remove 过程中如果有一个字符节点被其他键共用，则从该字符节点开始往上的路径节点都不能被删除，
     * 因此可以定义一个标志位 flag ，如果 flag 为 false，表示当前节点之下的路径上有未删除的节点 */
    bool Remove(const std::string &key)
    {
      latch_.WLock();
      bool flag = true;
      std::stack<unique_ptr<TrieNode> *> stk;
      auto cur = &this->root_;
      /* 若 key 存在，则将其节点路径存到栈中 */
      for (size_t i = 0; i < key.size(); ++i)
      {
        /* 不存在 key 节点路径，则返回 false */
      ...
      size_t idx = key.size() - 1; // idx 指向 key 中的字符
      /* Remove 处理末尾节点与非末尾节点有一些差异 */
      while (!stk.empty())
      {
        auto parent = stk.top();
        auto child = parent->get()->GetChildNode(key[idx]);
        /* 处理末尾节点 */
        if (stk.size() == key.size())
        {
    ...
    T GetValue(const std::string &key, bool *success)
    {
      ...
      /* 返回类型不是要求的 T 类型 */
      auto p = dynamic_cast<TrieNodeWithValue<T> *>(cur->get());
      if (p)
      {
        *success = true;
        latch_.RUnlock();
